...
def fix_pdf_spacing(text):
    """Fix missing spaces in PDF-extracted text using fast DP algorithm only.
    AI fixing is available via manual 'Check Data Quality' button."""
    if not text:
        return text
    # Use fast DP algorithm only - no AI calls during upload
    return fix_text_spacing(text)

def extract_text_from_pdf(pdf_path):
    """Extract text from PDF with page numbers and headers using PyMuPDF (better quality)."""
    import fitz  # PyMuPDF - better text extraction than pdfplumber
    ...
    for i, page in enumerate(doc):
        ...
        # Clean up soft hyphens and weird line breaks
...
def ingest_pdf(pdf_path, name, year=None, source=None):
    ...
    # NOTE: Spacing fixes are done via "Check Data Quality" button after upload
    # We don't do it here to avoid slowing down/timing out uploads
...
def search_guidelines(query, limit=50, model="claude-sonnet-4-20250514", guideline_ids=None):
    """Search for relevant guideline chunks using AI-powered hybrid search.
    
    Args:
        query: The search query
        limit: Max results to return
        model: AI model for query understanding
        guideline_ids: Optional list of guideline IDs to limit search to (None = all)
    """
    conn = get_db()
    ...
    print(f"Guideline filter: {guideline_ids if guideline_ids else 'ALL'}")
    
    results = []
    seen_ids = set()
    
    # Build guideline filter clause
    guideline_filter = ""
    guideline_params = []
    if guideline_ids and len(guideline_ids) > 0:
        placeholders = ','.join(['%s'] * len(guideline_ids))
        guideline_filter = f"AND g.id IN ({placeholders})"
        guideline_params = list(guideline_ids)
    ...
    if embedding:
        ...
        query_sql = f"""
            ...
            LIMIT %s
        """
        params = [emb_str] + guideline_params + [emb_str, limit]
    ...
    for term in ai_terms[:10]:  # Use top 10 AI terms
        if len(term) >= 3:
            query_sql = f"""
                ...
                LIMIT %s
            """
            params = [f'%{term}%'] + guideline_params + [limit // 2]
            cur.execute(query_sql, params)
...
def ask_claude(question, context_chunks, model="claude-sonnet-4-20250514"):
    ...
    for chunk in context_chunks:
        ...
        # Use content directly (PyMuPDF extracts clean text)
...
def ask_question():
    ...
    reformed = data.get('reformed', False)  # New parameter for AI reformatting
    deep_review = data.get('deep_review', False)  # New parameter for thorough chunk review
    guideline_ids = data.get('guideline_ids', None)  # Optional: limit to specific guidelines
    
    # Validate guideline_ids if provided
    if guideline_ids is not None:
        if not isinstance(guideline_ids, list):
            guideline_ids = None
        else:
            # Filter to only valid integers
            guideline_ids = [int(gid) for gid in guideline_ids if isinstance(gid, (int, str)) and str(gid).isdigit()]
            if len(guideline_ids) == 0:
                guideline_ids = None
    
    # Search for relevant chunks - use SELECTED MODEL for query understanding
    # Get more chunks if deep_review is enabled
    initial_limit = 100 if deep_review else 50
    chunks = search_guidelines(question, limit=initial_limit, model=model, guideline_ids=guideline_ids)
    ...
    print(f"Guideline IDs: {guideline_ids if guideline_ids else 'ALL'}")
    ...
    # If deep_review, have AI filter chunks for relevance in batches
    if deep_review and len(chunks) > 20:
        chunks = ai_filter_relevant_chunks(question, chunks, model)
        print(f"[Deep Review] Filtered to {len(chunks)} relevant chunks")
    
    # Get answer from Claude/GPT
    result = ask_claude(question, chunks, model)
    
    # If reformed requested, reorganize the answer with AI
    if reformed and result.get('answer'):
        result = reform_answer_with_ai(question, result, model)
    
    result['reformed'] = reformed
    result['deep_review'] = deep_review
    result['guideline_ids'] = guideline_ids
    return jsonify(result)


def ai_filter_relevant_chunks(question, chunks, model):
    """Have AI review ALL chunks in batches and pick the most relevant ones."""
    
    BATCH_SIZE = 20  # Review 20 chunks at a time
    relevant_chunks = []
    
    print(f"[Deep Review] Starting review of {len(chunks)} chunks in batches of {BATCH_SIZE}")
    
    for batch_start in range(0, len(chunks), BATCH_SIZE):
        batch = chunks[batch_start:batch_start + BATCH_SIZE]
        batch_num = batch_start // BATCH_SIZE + 1
        
        # Build batch content for review
        batch_content = []
        for i, chunk in enumerate(batch):
            chunk_id = batch_start + i
            batch_content.append(f"""[CHUNK {chunk_id}]
Guideline: {chunk.get('guideline_name', 'Unknown')}
Page: {chunk.get('page_number', '?')}
Content: {chunk['content'][:800]}{'...' if len(chunk['content']) > 800 else ''}
[/CHUNK {chunk_id}]""")
        
        review_prompt = f"""You are reviewing medical guideline chunks to find ones relevant to a clinical question.
...
TASK: List ONLY the chunk IDs that are DIRECTLY relevant to answering this question.
- Include chunks with recommendations, guidelines, or data that help answer the question
- Exclude chunks that are only tangentially related or don't add value
- Be inclusive - if in doubt, include it

RESPOND WITH ONLY A COMMA-SEPARATED LIST OF RELEVANT CHUNK IDs (e.g., "0, 3, 5, 12") OR "NONE" IF NO CHUNKS ARE RELEVANT."""
        ...
        try:
            if model.startswith('gpt'):
                ...
                if openai_key:
                    ...
                    if resp.status_code == 200:
                        ...
                        relevant_ids = parse_chunk_ids(result, batch_start, len(batch))
                        for idx in relevant_ids:
                            if 0 <= idx < len(chunks):
                                relevant_chunks.append(chunks[idx])
                        print(f"[Deep Review] Batch {batch_num}: found {len(relevant_ids)} relevant chunks")
            else:
                # Use Claude
                if claude:
                    response = claude.messages.create(
                        model="claude-haiku-3-5-20241022",  # Use fast model for filtering
                        max_tokens=200,
                        messages=[{'role': 'user', 'content': review_prompt}]
                    )
            ...
            print(f"[Deep Review] Batch {batch_num} error: {e}, keeping all chunks from batch")
            relevant_chunks.extend(batch)
    
    # If no chunks were marked relevant, return original top chunks
    if not relevant_chunks:
        print("[Deep Review] No relevant chunks found, using top 20 from original search")
        return chunks[:20]
    ...
    for chunk in relevant_chunks:
        chunk_key = (chunk.get('id'), chunk.get('page_number'))
        if chunk_key not in seen:
            seen.add(chunk_key)
            unique_chunks.append(chunk)
    
    return unique_chunks[:50]  # Cap at 50 most relevant


def parse_chunk_ids(result_text, batch_start, batch_size):
    """Parse chunk IDs from AI response."""
    ...
    for num_str in numbers:
        try:
            num = int(num_str)
            # Validate it's within the expected range
            if batch_start <= num < batch_start + batch_size:
                ids.append(num)
        except:
            pass
...
def reform_answer_with_ai(question, original_result, model):
    """Use AI to reorganize and clarify the answer while keeping ALL information."""
    
    # Gather all source content
    all_content = []
    for source in original_result.get('sources', []):
        guideline = source.get('guideline', 'Unknown')
        page = source.get('page', '?')
        text = source.get('full_text', source.get('excerpt', ''))
        all_content.append(f"[{guideline}, Page {page}]:\n{text}")
    
    combined_content = "\n\n---\n\n".join(all_content)
    
    reform_prompt = f"""You are reorganizing clinical guideline information to make it clearer and easier to read.

ORIGINAL QUESTION: {question}

RAW GUIDELINE CONTENT:
{combined_content}

YOUR TASK:
1. Reorganize this information into a clear, logical structure
2. **KEEP ALL INFORMATION** - Do NOT summarize or omit any details
3. Group related recommendations together
4. Use clear headings and bullet points where appropriate
5. Preserve all Class/LOE designations (e.g., Class I, Class IIa, LOE A, LOE B)
6. Preserve all specific numbers, percentages, and thresholds
7. Preserve all drug names, dosages, and durations
8. Include page references in parentheses where helpful
...
- Keep Class/LOE in bold like: **Class I, LOE A**

IMPORTANT: Your response should contain ALL the clinical details from the source material, just organized better. Do not leave anything out."""
    ...
    try:
        if model.startswith('gpt'):
            ...
            resp = requests.post(
                ...
                headers={'Authorization': f'Bearer {openai_key}', 'Content-Type': 'application/json'},
                json={
                    'model': 'gpt-4o' if model == 'gpt-5.2' else model,
                    'messages': [
                        {'role': 'system', 'content': 'You are a medical information organizer. Keep ALL details.'},
            ...
            if resp.status_code == 200:
                reformed_answer = resp.json()['choices'][0]['message']['content']
                original_result['answer'] = reformed_answer
                original_result['model'] = f"{model} (reformed)"
        else:
            # Use Claude
            if not claude:
                return original_result
            ...
            reformed_answer = response.content[0].text
            original_result['answer'] = reformed_answer
            original_result['model'] = f"{model} (reformed)"
            
            # Update token usage
            original_result['usage']['input_tokens'] += response.usage.input_tokens
            original_result['usage']['output_tokens'] += response.usage.output_tokens
            
    except Exception as e:
        print(f"Reform error: {e}")
        # Return original if reforming fails
...
def check_spacing_issues():
    ...
    def is_false_positive(word):
        """Check if a long word is actually valid (not a spacing issue)."""
        word_lower = word.lower()
        
        # Skip URLs and DOIs
        if any(x in word_lower for x in ['http', 'doi.org', 'www.', '.com', '.org', '.gov', '.edu', '.aspx', '.html', '.php']):
            return True
        
        # Skip table of contents dots (........)
        if '..' in word or word.count('.') > 3:
            return True
        
        # Skip words with slashes (paths, medical terms like phaeochromocytoma/paraganglioma)
        if '/' in word and len(word.split('/')) <= 3:
            return True
        
        # Skip words that are mostly numbers/special chars (like reference numbers)
        alpha_count = sum(1 for c in word if c.isalpha())
        if alpha_count < len(word) * 0.6:  # Less than 60% letters
            return True
        
        # Skip known long medical terms (they're valid!)
        valid_long_terms = [
            'electrocardiogram', 'electrocardiographic', 'echocardiography', 
            'cardiomyopathy', 'cardiomyopathies', 'phaeochromocytoma', 
            'paraganglioma', 'revascularization', 'catheterization',
            'recommendations', 'pharmacogenetics', 'anticoagulation'
        ]
        if any(term in word_lower for term in valid_long_terms):
            return True
    ...
    for chunk in chunks:
        ...
        real_issues = [w for w in long_words if not is_false_positive(w)]
        ...
        if real_issues:
            ...
            if len(issues_by_guideline[gid]['examples']) < 3:
                issues_by_guideline[gid]['examples'].append({
                    'chunk_id': chunk['id'],
                    'sample': real_issues[0][:80] if real_issues else ''
                })
...
def reextract_all_guidelines():
    """
    MANUAL ENDPOINT - Re-extract ALL PDFs using PyMuPDF (better quality).
    
    This will:
    1. Loop through all guidelines with stored PDFs
    2. DELETE all old chunks for each guideline
    3. Re-extract text using PyMuPDF
    4. Create new clean chunks
    5. Generate new embeddings
    
    Call with: curl -X POST https://your-server/api/reextract-all
    
    WARNING: This is slow and will take several minutes!
    """
    import tempfile
    import os
    import gc
    ...
    # Get all guidelines with stored PDFs
    cur.execute("""
        SELECT id, name, pdf_data, filename
        FROM guidelines 
    ...
    print(f"[Re-extract] Starting re-extraction of {len(guidelines)} guidelines...")
    ...
    for g in guidelines:
        ...
        print(f"[Re-extract] Processing: {gname} (ID: {gid})")
        
        if not pdf_data:
            results['skipped'].append({'id': gid, 'name': gname, 'reason': 'No PDF data'})
            continue
        ...
        try:
            ...
            # Extract text using PyMuPDF (the GOOD extractor!)
            ...
            if not pages:
                results['failed'].append({'id': gid, 'name': gname, 'error': 'No text extracted'})
                continue
            
            # Step 1: DELETE all old chunks for this guideline
            cur.execute("DELETE FROM chunks WHERE guideline_id = %s", (gid,))
            deleted_count = cur.rowcount
            conn.commit()
            print(f"[Re-extract] Deleted {deleted_count} old chunks for {gname}")
            
            # Step 2: Create new chunks from clean text
            new_chunks = []
            for page in pages:
                page_chunks = chunk_text(page['text'], page['page_number'])
                new_chunks.extend(page_chunks)
            
            # Step 3: Insert new chunks with embeddings
            chunks_created = 0
            for chunk in new_chunks:
                ...
                is_ref = is_reference_chunk(chunk['content'])
                
                cur.execute("""
                    INSERT INTO chunks (guideline_id, content, page_number, embedding, is_reference)
                    VALUES (%s, %s, %s, %s::vector, %s)
                """, (gid, chunk['content'], chunk['page_number'], emb_str, is_ref))
                chunks_created += 1
            ...
            results['success'].append({
                'id': gid, 
                'name': gname, 
                'old_chunks': deleted_count,
                'new_chunks': chunks_created
            })
            results['processed'] += 1
            print(f"[Re-extract] SUCCESS: {gname} - {chunks_created} new chunks created")
            
            # Free memory
            gc.collect()
            
            # Wait 30 seconds before next guideline (avoid overloading server/API)
            import time
            remaining = len(guidelines) - results['processed'] - len(results['failed']) - len(results['skipped'])
            if remaining > 0:
                print(f"[Re-extract] Waiting 30 seconds... ({remaining} guidelines remaining)")
                time.sleep(30)
            
        except Exception as e:
            results['failed'].append({'id': gid, 'name': gname, 'error': str(e)})
            print(f"[Re-extract] FAILED: {gname} - {str(e)}")
            conn.rollback()
    
    cur.close()
    conn.close()
    
    print(f"[Re-extract] COMPLETE! Processed: {results['processed']}, Failed: {len(results['failed'])}")
    
    return jsonify(results)


@app.route('/api/reextract-one/<int:guideline_id>', methods=['POST'])
def reextract_one_guideline(guideline_id):
    """Re-extract a single guideline's PDF. Useful for testing."""
    import tempfile
    import os
    ...
    if not g:
        cur.close()
        conn.close()
        return jsonify({'error': 'Guideline not found or has no PDF'}), 404
    ...
    try:
        ...
        for i, page in enumerate(doc):
            ...
            if text.strip():
                ...
                # DEBUG: Log first 200 chars of each page
                print(f"[Re-extract ONE] Page {i+1} preview: {text[:200]}")
        doc.close()
        os.unlink(tmp_path)
        ...
        # DEBUG: Check if text is clean
        sample = pages[1]['text'] if len(pages) > 1 else pages[0]['text']
        print(f"[Re-extract ONE] Has 'characterized': {'characterized' in sample}")
        print(f"[Re-extract ONE] Has 'ch ar a cter': {'ch ar a cter' in sample}")
        ...
        import traceback
        print(f"[Re-extract ONE] ERROR: {str(e)}")
        print(f"[Re-extract ONE] Traceback: {traceback.format_exc()}")
        try:
            cur.close()
            conn.close()
        except:
...
@app.route('/api/delete-chunks/<int:guideline_id>', methods=['POST'])
def delete_chunks(guideline_id):
    """Delete all chunks for a specific guideline. Use before re-extracting."""
    conn = get_db()
    cur = conn.cursor(row_factory=dict_row)
    ...
    # Delete chunks
    cur.execute("DELETE FROM chunks WHERE guideline_id = %s", (guideline_id,))
...
@app.route('/api/clear-pptx-cache', methods=['POST'])
def clear_pptx_cache():
    """Clear all cached PPTX/HTML data from pptx_jobs table."""
    conn = get_db()
    cur = conn.cursor()
    
    # Clear cached pptx_data and html_data
    cur.execute("UPDATE pptx_jobs SET pptx_data = NULL, html_data = NULL")
    updated = cur.rowcount
    conn.commit()
    
    # Also delete old completed jobs
    cur.execute("DELETE FROM pptx_jobs WHERE status = 'completed'")
    deleted = cur.rowcount
    conn.commit()
    ...
    return jsonify({
        'success': True,
        'cache_cleared': updated,
        'jobs_deleted': deleted
    })


@app.route('/api/check-pptx-cache')
def check_pptx_cache():
    """Check what's in the PPTX cache."""
    conn = get_db()
    cur = conn.cursor(row_factory=dict_row)
    
    cur.execute("""
        SELECT j.id, j.guideline_id, g.name as guideline_name, j.topic, j.status,
               j.created_at, j.completed_at,
               CASE WHEN j.pptx_data IS NOT NULL THEN TRUE ELSE FALSE END as has_pptx,
               CASE WHEN j.html_data IS NOT NULL THEN TRUE ELSE FALSE END as has_html,
               CASE WHEN j.result_data IS NOT NULL THEN TRUE ELSE FALSE END as has_result_data,
               LENGTH(j.result_data) as result_data_size
        FROM pptx_jobs j
        LEFT JOIN guidelines g ON j.guideline_id = g.id
    ...
    return jsonify({
        'total_cached_jobs': len(jobs),
        'jobs': jobs
    })


@app.route('/api/view-pptx-result/<int:job_id>')
def view_pptx_result(job_id):
    """View the result_data for a specific job to debug content."""
    conn = get_db()
    cur = conn.cursor(row_factory=dict_row)
    
    cur.execute("SELECT result_data FROM pptx_jobs WHERE id = %s", (job_id,))
    job = cur.fetchone()
    ...
    if not job or not job['result_data']:
        return jsonify({'error': 'No result data found'}), 404
    
    import json
    result = json.loads(job['result_data'])
    
    # Return first 2000 chars of each section to check for broken text
    preview = {}
    for key, items in result.items():
        if isinstance(items, list) and len(items) > 0:
            sample = str(items[0])[:500] if items else ''
            preview[key] = {
                'count': len(items),
                'sample': sample
            }
...
@app.route('/api/delete-all-chunks', methods=['POST'])
def delete_all_chunks():
    """Delete ALL chunks from ALL guidelines. Nuclear option before full re-extract."""
    conn = get_db()
    cur = conn.cursor()
...
@app.route('/api/test-extraction/<int:guideline_id>')
def test_extraction(guideline_id):
    """Debug endpoint: Test PDF extraction for a guideline and show raw text."""
    import tempfile
    import os
    ...
    for i, page in enumerate(doc):
        ...
        pages_text.append({
            ...
            'has_characterized': 'characterized' in text,
            'has_broken_char': 'ch ar a cter' in text
        })
    doc.close()
    os.unlink(tmp_path)
...
def fix_guideline_spacing(guideline_id):
    ...
    # Ensure spacing_jobs table exists
    cur.execute("""
        CREATE TABLE IF NOT EXISTS spacing_jobs (
            id SERIAL PRIMARY KEY,
            guideline_id INT REFERENCES guidelines(id) ON DELETE CASCADE,
...
def process_spacing_job(job_id, guideline_id, chunks_to_fix):
    ...
    BATCH_SIZE = 5  # Keep small to avoid API timeouts
    ...
    for batch_start in range(0, len(chunks_to_fix), BATCH_SIZE):
        ...
        try:
            ...
            # Single AI call for entire batch - use streaming to avoid timeout
            if claude:
                result_text = ""
...
def get_spacing_job_status(job_id):
    ...
    try:
        ...
        cur.close()
        conn.close()
        return jsonify({'error': f'Table not ready: {str(e)}'}), 404
    ...
    for key, config in section_config.items():
        ...
        <section class="section" id="{key}">
            ...
            for i, item in enumerate(items, 1):
                ...
                # NOTE: Do NOT call fix_pdf_spacing here - the text from result_data is already clean!
                # The fix_pdf_spacing function was BREAKING clean text by trying to segment long medical terms.
