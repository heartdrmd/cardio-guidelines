...
def generate_pptx(guideline_id):
    """Generate a comprehensive PPTX extracting ALL recommendations from a guideline."""
    import json
    ...
    # Get ALL chunks - EXCLUDE references
    if topic:
        cur.execute("""
    ...
    print(f"=== PPTX MULTI-PASS EXTRACTION ===")
    print(f"Guideline: {guideline['name']}")
    print(f"Total chunks: {len(all_chunks)}")
    print(f"Topic filter: {topic or 'None (full guideline)'}")
    
    if not claude:
        return jsonify({'error': 'Anthropic API key not configured'}), 500
    
    # MULTI-PASS EXTRACTION
    # Process chunks in batches to get ALL recommendations
    CHUNKS_PER_BATCH = 30  # ~30 chunks per API call
    all_recommendations = {
        'class_1': [],
        'class_2a': [],
        'class_2b': [],
    ...
    for i in range(0, len(all_chunks), CHUNKS_PER_BATCH):
        ...
        print(f"Processing batch {batch_num}: chunks {i+1}-{i+len(batch_chunks)} of {len(all_chunks)}")
        
        extraction_prompt = f"""Extract ALL clinical recommendations from this section of the {guideline['name']} guideline{topic_text}.

CONTENT (Pages {batch_chunks[0]['page_number']}-{batch_chunks[-1]['page_number']}):
{context}

Extract EVERY recommendation you find. Look for:
- Class I recommendations (Recommended)
- Class IIa recommendations (Reasonable/Should be considered)
- Class IIb recommendations (May be considered)
- Class III recommendations (Not recommended/Harmful)

Return JSON format:
{{
    "class_1": [
        {{"recommendation": "exact text", "loe": "A/B/C", "page": 123}}
    ],
    "class_2a": [...],
    "class_2b": [...],
    "class_3": [...]
...
Rules:
1. Extract EVERY recommendation - do NOT skip any
2. Use the EXACT wording from the guideline
3. Include Level of Evidence (LOE) if mentioned
4. Include page number
5. If no recommendations in this section, return empty arrays
6. Return ONLY valid JSON"""
        ...
        try:
            ...
            if start >= 0 and end > start:
                ...
                # Merge into all_recommendations
                for class_key in ['class_1', 'class_2a', 'class_2b', 'class_3']:
                    ...
                    if batch_recs:
                        ...
                        print(f"  Batch {batch_num}: Found {len(batch_recs)} {class_key} recommendations")
            else:
                print(f"  Batch {batch_num}: No valid JSON found")
    ...
    print(f"Total recommendations extracted: {total_recs}")
    print(f"  Class I: {len(all_recommendations['class_1'])}")
    print(f"  Class IIa: {len(all_recommendations['class_2a'])}")
    print(f"  Class IIb: {len(all_recommendations['class_2b'])}")
    print(f"  Class III: {len(all_recommendations['class_3'])}")
    print(f"Total tokens: {total_input_tokens + total_output_tokens}")
    
    if total_recs == 0:
        return jsonify({'error': 'No recommendations found in guideline'}), 404
    ...
    if pptx_path:
        ...
        response = make_response(send_file(
            pptx_path,
            as_attachment=True,
            download_name=f"{guideline['name'].replace(' ', '_')}{'_' + topic if topic else ''}.pptx",
