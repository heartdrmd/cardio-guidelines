...
def download_pptx_job_text(job_id):
    """Download completed extraction as plain text."""
    conn = get_db()
    cur = conn.cursor(row_factory=dict_row)
    ...
    lines = []
    lines.append("=" * 80)
    lines.append(job['guideline_name'].upper())
    if job['topic']:
        lines.append(f"Topic: {job['topic']}")
    lines.append(f"Source: {job['source'] or 'N/A'} ({job['year'] or 'N/A'})")
    lines.append("=" * 80)
    lines.append("")
    ...
    for section_key, section_title in section_titles.items():
        ...
        if items:
            ...
            for i, item in enumerate(items, 1):
                ...
                if item_class:
                    meta_parts.append(f"Class {item_class}")
                if loe:
                    meta_parts.append(f"LOE: {loe}")
                if meta_parts:
    ...
    lines.append("=" * 80)
    lines.append(f"Total items extracted: {total_items}")
    lines.append(f"AI Model: {job['model']}")
    lines.append(f"Cost: ${job['cost']:.4f} | Tokens: {job['tokens']:,}")
    lines.append("=" * 80)
    
    content = "\n".join(lines)
    
    filename = f"{job['guideline_name'].replace(' ', '_')}{'_' + job['topic'] if job['topic'] else ''}.txt"
...
4. EXCLUDE only: author lists, journal references, acknowledgments, disclosures
5. Include ALL tables, criteria, algorithms as detailed text

Return ONLY valid JSON:
{{
    "recommendations": [{{"text": "full text", "class": "I/IIa/IIb/III", "loe": "A/B/C", "page": 15}}],
    "definitions": [{{"text": "full text", "page": 12}}],
    "epidemiology": [{{"text": "full text", "page": 10}}],
    "pathophysiology": [{{"text": "full text", "page": 11}}],
    "diagnosis": [{{"text": "full text", "page": 14}}],
    "treatment": [{{"text": "full text", "page": 16}}],
    "management": [{{"text": "full text", "page": 18}}],
    "special_populations": [{{"text": "full text", "page": 20}}],
    "key_points": [{{"text": "full text", "page": 22}}]
}}"""
        ...
        if total_items == 0:
            cur.execute("""
                UPDATE pptx_jobs 
                SET status = 'failed', error = 'No content found'
                WHERE id = %s
            """, (job_id,))
        else:
            summary_json = {
        ...
        for section_key, section_title in SECTION_TITLES.items():
            ...
            for slide_num, start_idx in enumerate(range(0, len(items), items_per_slide)):
                ...
                for i, item in enumerate(slide_items):
                    ...
                    text_box = content_slide.shapes.add_textbox(Inches(1.0), Inches(current_top), Inches(11.5), Inches(2.5))
                    text_frame = text_box.text_frame
                    text_frame.word_wrap = True
                    text_para = text_frame.paragraphs[0]
                    text_para.text = item_text[:800] if len(item_text) > 800 else item_text
                    ...
                    if meta_parts:
                        meta_box = content_slide.shapes.add_textbox(Inches(1.0), Inches(current_top + 2.3), Inches(11), Inches(0.3))
                        meta_frame = meta_box.text_frame
                        meta_para = meta_frame.paragraphs[0]
                        meta_para.text = " | ".join(meta_parts)
                        meta_para.font.size = Pt(12)
                        meta_para.font.italic = True
                        meta_para.font.color.rgb = RGBColor(148, 163, 184)
