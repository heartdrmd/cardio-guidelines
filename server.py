...
def ingest_pdf(pdf_path, name, year=None, source=None):
    ...
    # Fix spacing issues in chunks BEFORE saving to database
    # This ensures clean data is stored, not patched later
    chunks_fixed = 0
    for chunk in all_chunks:
        if check_text_needs_spacing_fix(chunk['content']):
            print(f"  Fixing spacing in chunk from page {chunk['page_number']}...")
            chunk['content'] = fix_text_spacing_with_ai(chunk['content'])
            chunks_fixed += 1
    
    if chunks_fixed > 0:
        print(f"  Fixed spacing issues in {chunks_fixed} chunks using AI")
...
def process_spacing_job(job_id, guideline_id, chunks_to_fix):
    """Background worker to fix spacing in chunks."""
    ...
    for i, chunk in enumerate(chunks_to_fix):
        try:
            # Try AI fix first
            fixed_content = fix_text_spacing_with_ai(chunk['content'])
            ...
            if fixed_content and fixed_content != chunk['content']:
                ...
                fixed_count += 1
                print(f"[Spacing Job {job_id}] Fixed chunk {chunk['id']} ({i+1}/{len(chunks_to_fix)})")
        except Exception as e:
            print(f"[Spacing Job {job_id}] AI fix failed for chunk {chunk['id']}: {e}")
            # Fall back to DP algorithm
            try:
                ...
                print(f"[Spacing Job {job_id}] DP fix also failed: {e2}")
        
        # Update progress
        progress = int((i + 1) / len(chunks_to_fix) * 100)
        cur.execute(
