...
def create_pptx(summary, guideline, topic=None):
    ...
    try:
        ...
        # Color scheme - using RGBColor (not RgbColor)
        ...
        class_info = {
            'class_1': {'name': 'CLASS I (Recommended)', 'color': RGBColor(16, 185, 129), 'desc': 'Benefit >>> Risk'},
            'class_2a': {'name': 'CLASS IIa (Reasonable)', 'color': RGBColor(59, 130, 246), 'desc': 'Benefit >> Risk'},
            'class_2b': {'name': 'CLASS IIb (May Consider)', 'color': RGBColor(245, 158, 11), 'desc': 'Benefit ≥ Risk'},
            'class_3': {'name': 'CLASS III (Not Recommended)', 'color': RGBColor(239, 68, 68), 'desc': 'No Benefit or Harm'},
        }
        
        # Create slides for each class that has recommendations
        for class_key, info in class_info.items():
            recommendations = summary.get(class_key, [])
            if not recommendations:
                continue
            
            # May need multiple slides if many recommendations
            recs_per_slide = 4
            for slide_num, start_idx in enumerate(range(0, len(recommendations), recs_per_slide)):
                ...
                header_box = content_slide.shapes.add_textbox(Inches(0.5), Inches(0.3), Inches(12.333), Inches(0.8))
                header_frame = header_box.text_frame
                header_para = header_frame.paragraphs[0]
                page_indicator = f" ({slide_num + 1}/{-(-len(recommendations)//recs_per_slide)})" if len(recommendations) > recs_per_slide else ""
                header_para.text = f"{info['name']}{page_indicator}"
                header_para.font.size = Pt(28)
                header_para.font.bold = True
                header_para.font.color.rgb = info['color']
                
                # Description
                desc_box = content_slide.shapes.add_textbox(Inches(0.5), Inches(1.0), Inches(12.333), Inches(0.4))
                desc_frame = desc_box.text_frame
                desc_para = desc_frame.paragraphs[0]
                desc_para.text = info['desc']
                desc_para.font.size = Pt(14)
                desc_para.font.italic = True
                
                # Recommendations
                current_top = 1.5
                for rec in slide_recs:
                    rec_text = rec.get('recommendation', '') if isinstance(rec, dict) else str(rec)
                    loe = rec.get('loe', '') if isinstance(rec, dict) else ''
                    
                    # Recommendation box
                    rec_box = content_slide.shapes.add_textbox(Inches(0.5), Inches(current_top), Inches(11.5), Inches(1.2))
                    rec_frame = rec_box.text_frame
                    rec_frame.word_wrap = True
                    rec_para = rec_frame.paragraphs[0]
                    rec_para.text = f"• {rec_text}"
                    rec_para.font.size = Pt(16)
                    
                    # LOE badge
                    if loe:
                        loe_para = rec_frame.add_paragraph()
                        loe_para.text = f"   LOE: {loe}"
