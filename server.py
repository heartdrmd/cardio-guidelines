...
def fix_guideline_spacing(guideline_id):
    """Re-process a guideline's chunks to fix spacing issues using AI."""
    conn = get_db()
    cur = conn.cursor(row_factory=dict_row)
    ...
    # Get all chunks for this guideline
    ...
    for chunk in chunks:
        ...
        if check_text_needs_spacing_fix(content):
            # Use AI to fix
            fixed_content = fix_text_spacing_with_ai(content)
