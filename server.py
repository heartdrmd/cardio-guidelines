...
def ingest_pdf(pdf_path, name, year=None, source=None):
    ...
    for page in pages:
        ...
        # and contain numbered references pattern
...
def start_pptx_job(guideline_id):
    ...
    total_batches = (chunk_count + 49) // 50  # 50 chunks per batch
...
def download_pptx_job(job_id):
    ...
    # Parse result and generate PPTX
    import json
    ...
    if pptx_path:
        ...
        response = make_response(send_file(
            pptx_path,
            as_attachment=True,
            download_name=f"{job['guideline_name'].replace(' ', '_')}{'_' + job['topic'] if job['topic'] else ''}.pptx",
...
@app.route('/api/pptx-job/<int:job_id>/download-text')
def download_pptx_job_text(job_id):
    """Download completed extraction as markdown text."""
    conn = get_db()
    cur = conn.cursor(row_factory=dict_row)
    ...
    lines = []
    lines.append(f"# {job['guideline_name']}")
    if job['topic']:
        lines.append(f"## Topic: {job['topic']}")
    lines.append(f"\n*Source: {job['source'] or 'N/A'} ({job['year'] or 'N/A'})*")
    lines.append(f"\n---\n")
    
    class_info = {
        'class_1': ('CLASS I - Recommended', 'Benefit >>> Risk'),
        'class_2a': ('CLASS IIa - Reasonable', 'Benefit >> Risk'),
        'class_2b': ('CLASS IIb - May Be Considered', 'Benefit ≥ Risk'),
        'class_3': ('CLASS III - Not Recommended', 'No Benefit or Harm'),
    }
    
    total_recs = 0
    for class_key, (title, desc) in class_info.items():
        recs = summary.get(class_key, [])
        if recs:
            lines.append(f"\n## {title}")
            lines.append(f"*{desc}*\n")
            for i, rec in enumerate(recs, 1):
                ...
                if loe or page:
                    lines.append(f"   - *LOE: {loe}*" + (f" *(Page {page})*" if page else ""))
                lines.append("")
                total_recs += 1
    
    lines.append(f"\n---")
    lines.append(f"*Total recommendations extracted: {total_recs}*")
    lines.append(f"*Generated with AI model: {job['model']}*")
    lines.append(f"*Cost: ${job['cost']:.4f} | Tokens: {job['tokens']:,}*")
    
    content = "\n".join(lines)
    
    filename = f"{job['guideline_name'].replace(' ', '_')}{'_' + job['topic'] if job['topic'] else ''}.md"
...
def process_pptx_job(job_id):
    ...
    try:
        ...
        # Process in batches
        CHUNKS_PER_BATCH = 50
        all_recommendations = {'class_1': [], 'class_2a': [], 'class_2b': [], 'class_3': []}
        ...
        for i in range(0, len(all_chunks), CHUNKS_PER_BATCH):
            ...
            extraction_prompt = f"""You are extracting clinical recommendations from a medical guideline for a presentation.

THIS IS NOT A SUMMARY. You must extract EVERY SINGLE recommendation EXACTLY as written.

GUIDELINE: {guideline['name']}
SECTION: Pages {batch_chunks[0]['page_number']}-{batch_chunks[-1]['page_number']}
{f"TOPIC FOCUS: {topic}" if topic else ""}

CONTENT:
{context}

TASK: Find and extract ALL clinical recommendations. Look for:
- Class I (Recommended / Is indicated / Should be performed)
- Class IIa (Is reasonable / Can be useful / Should be considered)  
- Class IIb (May be considered / May be reasonable / Usefulness uncertain)
- Class III (Not recommended / Is not indicated / Should not / Harmful)

IMPORTANT:
1. Extract the COMPLETE recommendation text - do NOT summarize or shorten
2. Include Level of Evidence (LOE: A, B, C, B-R, B-NR, C-LD, C-EO) if mentioned
3. Include page number where found
4. If NO recommendations in this section, return empty arrays
5. Do NOT make up recommendations - only extract what is explicitly stated

Return ONLY valid JSON in this format:
{{"class_1": [{{"recommendation": "full exact text", "loe": "A", "page": 15}}], "class_2a": [...], "class_2b": [...], "class_3": [...]}}"""
            ...
            try:
                if model.startswith('gpt-'):
                    ...
                    if resp.status_code == 200:
                        ...
                        print(f"Batch {batch_num} OpenAI error: {resp.text}")
                        continue
                ...
                if start >= 0 and end > start:
                    ...
                    for class_key in ['class_1', 'class_2a', 'class_2b', 'class_3']:
                        batch_recs = batch_json.get(class_key, [])
                        if batch_recs:
                            all_recommendations[class_key].extend(batch_recs)
                            print(f"  Found {len(batch_recs)} {class_key}")
        ...
        if total_recs == 0:
            cur.execute("""
                UPDATE pptx_jobs 
                SET status = 'failed', error = 'No recommendations found'
                WHERE id = %s
            """, (job_id,))
        else:
            summary_json = {
